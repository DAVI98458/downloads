...
local trackedDangers = {} -- animatronics que entraram na zona em movimento
local dangerCooldowns = {} -- [anim] = timestamp quando saiu da zona (aguarda 3s antes de limpar)
...
function resetAutoHide()
    ...
    dangerCheckPos = nil
    table.clear(trackedDangers)
    table.clear(dangerCooldowns)
...
RunService.Heartbeat:Connect(function()
    ...
    for _,anim in ipairs(AnimFolder:GetChildren()) do
        ...
        if p then
            ...
            if hiding then
                -- Dentro do armário: rastreia quem entrou em movimento
                if dist <= 29 then
                    if velocity > 1 then
                        trackedDangers[anim] = true
                    end
                    -- Voltou pra zona: cancela cooldown de saída
                    dangerCooldowns[anim] = nil

                    if trackedDangers[anim] then
                        danger = true
                        break
                    end
                else
                    -- Saiu da zona: inicia cooldown de 3s antes de limpar
                    if trackedDangers[anim] and not dangerCooldowns[anim] then
                        dangerCooldowns[anim] = os.clock()
                    end
                    -- Só remove depois de 3s fora da zona
                    if dangerCooldowns[anim] and (os.clock() - dangerCooldowns[anim] >= 3) then
                        trackedDangers[anim] = nil
                        dangerCooldowns[anim] = nil
                    end
                    -- Ainda no cooldown: continua sendo perigo
                    if trackedDangers[anim] then
                        danger = true
                        break
                    end
                ...
                -- Fora do armário: comportamento original (velocity > 1 + wall check)
                if velocity > 1 and dist <= 29 then
