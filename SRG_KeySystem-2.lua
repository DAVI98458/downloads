...
local HWID_MOD = 2176782336  -- 36^6 as integer (avoids float precision bug)
...
local function hwidHash(str)
    ...
    local result = toBase36(math.floor(h))
    while #result < 6 do result = "0" .. result end
    return result
end

local BIND_FILE  = "srg_bind.txt"

local function saveBinding(key, hwid)
    pcall(function() writefile(BIND_FILE, key .. "|" .. hwid) end)
end

local function loadBinding()
    local ok, val = pcall(function() return readfile(BIND_FILE) end)
...
local function verifyKey(key)
    if type(key) ~= "string" then return false, "invalid" end

    local dayPart, hwidPart, rand = key:match("^SRG%-([A-Z0-9]+)%-([A-Z0-9]+)%-([A-Z0-9]+)$")
    if not dayPart or not hwidPart or not rand then return false, "invalid" end
    if #hwidPart ~= 6 or #rand ~= 4 then return false, "invalid" end

    -- day check
    local dayStr    = dayPart:sub(1, -2)
    local checkChar = dayPart:sub(-1):upper()
    local keyDay    = fromBase36(dayStr)
    if not keyDay then return false, "invalid" end

    local currentDay = math.floor(os.time() / 86400)
    if math.abs(currentDay - keyDay) > 1 then return false, "expired" end

    local sum = 0
    for i = 1, #dayStr do sum = sum + dayStr:byte(i) end
    if checkChar ~= B36:sub((sum % 36) + 1, (sum % 36) + 1) then return false, "invalid" end

    -- HWID check
    local myHwid = "NO_HWID"
    pcall(function() myHwid = tostring(fGetHwid()) end)

    local noHwidHash = hwidHash("NO_HWID")

    if hwidPart == noHwidHash then
        -- unbound key — check if we have a local binding for it
        local boundKey, boundHwid = loadBinding()
        if boundKey == key then
            -- already claimed — check if this is the same person
            if boundHwid == myHwid then
                return true, "ok"
            else
                return false, "claimed"
            end
        end
        -- not yet claimed — valid, claim it now
        return true, "claim"
    else
        -- HWID-bound key
        if hwidPart ~= hwidHash(myHwid) then return false, "hwid_mismatch" end
        return true, "ok"
    end
end
...
local function buildKeyUI()
    ...
    checkBtn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            ...
            if valid then
                if reason == "claim" then
                    -- first use of unbound key — claim it
                    local myHwid = "NO_HWID"
                    pcall(function() myHwid = tostring(fGetHwid()) end)
                    saveBinding(key, myHwid)
                    saveKey(key)
                    -- open site so it saves HWID in localStorage
                    pcall(function()
                        local url = "https://randomscripts364.github.io/verified/?hwid="
                            .. game:GetService("HttpService"):UrlEncode(myHwid)
                        if syn and syn.openbrowser then syn.openbrowser(url)
                        elseif openbrowser then openbrowser(url) end
                    end)
                else
                    saveKey(key)
                end
                ...
                if reason == "expired" then
                    statusLbl.Text = "❌  Key expired. Get a new one at the site."
                elseif reason == "claimed" then
                    statusLbl.Text = "❌  This key is already claimed by another device."
                elseif reason == "hwid_mismatch" then
                    statusLbl.Text = "❌  Key not valid for this device."
                else
                    statusLbl.Text = "❌  Invalid key. Get one at the site."
                end
...
local valid, reason = verifyKey(savedKey or "")

if savedKey and valid then
    if HasScript then runScript() else showNoScriptMessage() end
else
