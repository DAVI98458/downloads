...
local function StartAntiVoid()
    ...
    AntiVoidConn = RunService.Heartbeat:Connect(function()
        ...
        -- Salva posição segura: Y > -50 e tem chão abaixo
        if pos.Y > -50 then
        ...
        if pos.Y < -100 then
            ...
            -- Move gradualmente em steps ao longo de 3 segundos
            task.spawn(function()
                local steps    = 30
                local interval = 3 / steps
                local startCF  = root.CFrame

                for i = 1, steps do
                    if not State.AntiVoidEnabled then break end
                    local alpha = i / steps
                    -- Interpolação suave
