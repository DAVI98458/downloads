...
-- Tenta equipar o L106 do inventário usando 2 métodos diferentes via pcall
local function TryEquipL106()
    local char    = LocalPlayer.Character
    ...
    -- Verifica se já está equipado no character (não no backpack)
    local equippedTool = char:FindFirstChild("L106")
    if equippedTool then
        return true -- já equipado, não precisa fazer nada
    end

    local toolInBackpack = backpack:FindFirstChild("L106")
    if not toolInBackpack then
        return false -- não tem no inventário
    end

    -- Método 1: EquipTool via Humanoid
    ...
    if ok1 then
        task.wait(0.15)
        -- Confirma se equipou
        if char:FindFirstChild("L106") then
            return true
        end
    end

    -- Método 2: Mover o Tool para o Character diretamente
...
local function FireL106()
    -- Só dispara se o Aimbot estiver ativo e com um alvo travado
    if not State.AimbotEnabled or not State.SelectedTarget then return end
    ...
    -- Tenta equipar se necessário
    local equipped = TryEquipL106()

    if not equipped then
        WindUI:Notify({
            Title    = "Auto Shoot",
            Content  = "L106 not found in inventory!",
            Duration = 3,
            Icon     = "x",
        })
    ...
    -- Aguarda o FireRemote aparecer na arma equipada
    local gun = char:FindFirstChild("L106")
    if not gun then return end

    local fireRemote = gun:FindFirstChild("Fire")
    if not fireRemote then return end

    -- Usa a posição atual do personagem como alvo do disparo
...
local function StartAutoShoot()
    ...
    AutoShootConn = task.spawn(function()
        while State.AutoShootEnabled do
            FireL106()
            task.wait(0.1) -- intervalo entre disparos (ajuste conforme necessário)
        end
    end)
end
...
-- AUTO SHOOT GUN TOGGLE (logo abaixo do Disable Fall Injury)
TabCombat:Toggle({
    Title = "Auto Shoot Gun",
    Desc  = "Equips L106 from inventory and auto fires at your current position",
    Value = false,
    Callback = function(enabled)
        State.AutoShootEnabled = enabled
        if enabled then
            -- Checa de cara se tem a arma antes de ativar
            local char    = LocalPlayer.Character
            local backpack = LocalPlayer:FindFirstChild("Backpack")
            local hasGun  = (char and char:FindFirstChild("L106") ~= nil)
                         or (backpack and backpack:FindFirstChild("L106") ~= nil)

            if not hasGun then
                WindUI:Notify({
                    Title    = "Auto Shoot",
                    Content  = "L106 is not in your inventory!",
                    Duration = 3,
                    Icon     = "x",
                })
                State.AutoShootEnabled = false
