...
local function StartAimbot()
    ...
    AimbotConn = RunService.Heartbeat:Connect(function()
        ...
        if lastLOS then
            ...
            Camera.CFrame = CFrame.lookAt(cf.Position, targetPos)
        end
    end)
end
...
-- ============================================================
local WATCH_ITEMS = { "Shield", "Katana", "L106", "Knife", "Briefcase", "Sledge", "Vest", "Bandage", "AS-VAL" }
local SpawnConns  = {}
...
local function IsBot(obj)
    ...
    if inZombies then return false end
    return true
end
...
-- ============================================================
-- Acessa diretamente workspace.NomeDoPlayer.HandlerName
...
-- ============================================================
TabMisc:Button({
    Title = "bypass Anti-cheat method 1 [Recommended and Best]",
    Desc  = "This removes some RemoteEvents related to the game, it may sometimes not work, I can't guarantee it",
    Callback = function()
