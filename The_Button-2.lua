...
local function WaitForProximityPrompt(obj, maxTime)
    ...
    while t < maxTime do
        ...
        -- Tenta no pai também (caso obj seja uma BasePart dentro de um Model)
...
local antiVoidRecovering = false

local function StopAntiVoid()
    if AntiVoidConn then AntiVoidConn:Disconnect() AntiVoidConn = nil end
    antiVoidRecovering = false
end
...
local function StartAntiVoid()
    ...
    AntiVoidConn = RunService.Heartbeat:Connect(function()
        ...
        if antiVoidRecovering then return end
        ...
        if pos.Y < -100 then
            ...
            WindUI:Notify({
                Title    = "Anti-Void",
                Content  = lastSafeCFrame and "Void detected! Teleported to safe position." or "Void detected! No safe position saved yet.",
                Duration = 2,
                Icon     = "shield",
            })
