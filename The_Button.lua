...
-- Verifica se o item está solto no Workspace (não dentro de Character ou Backpack)
local function IsItemFreeInWorld(obj)
    ...
    while ancestor and ancestor ~= game do
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            ...
            if bp and bp == ancestor then return false end
        end
...
local function GrabItem(itemName)
                ...
                if obj:IsA("BasePart") then
                    ...
                    if not itemPart then
                        for _, child in ipairs(obj:GetDescendants()) do
                            if child:IsA("BasePart") then itemPart = child break end
                        end
                    end
                end
                break
    ...
    if not itemPart then
        WindUI:Notify({ Title = "Error", Content = "Cannot locate " .. itemName, Duration = 2, Icon = "x" })
        return
    end
...
-- ANTI-VOID
-- Salva continuamente o último CFrame onde o player tinha chão sob os pés
...
local function StartAntiVoid()
    ...
    AntiVoidConn = RunService.Heartbeat:Connect(function()
        ...
        -- Atualiza a posição segura somente quando:
        -- 1) o player está acima do limiar do void (Y > -50)
        -- 2) tem chão sob o player
        if pos.Y > -50 then
        ...
        -- Detecta void: player abaixo de Y -100
        if pos.Y < -100 then
            if lastSafeCFrame then
                root.CFrame = lastSafeCFrame
                WindUI:Notify({
                    Title    = "Anti-Void",
                    Content  = "Void detected! Teleported to last safe position.",
                    Duration = 2,
                    Icon     = "shield",
                })
            else
                -- Sem posição segura salva, sobe o player como fallback
                root.CFrame = root.CFrame + Vector3.new(0, 60, 0)
                WindUI:Notify({
                    Title    = "Anti-Void",
                    Content  = "Void detected! No safe position saved yet.",
                    Duration = 2,
                    Icon     = "shield",
                })
            end
...
-- Tenta fazer o auto grab de um único objeto se os critérios forem atendidos
local function TryAutoGrab(obj)
    ...
    -- Deve estar solto no mundo (não no inventário/Character)
    if not IsItemFreeInWorld(obj) then return end

    -- Deve ter chão abaixo (não no void)
    if not ItemHasGround(obj) then return end

    -- Evita processar o mesmo objeto mais de uma vez ao mesmo tempo
    if autoGrabInProgress[obj] then return end
    autoGrabInProgress[obj] = true

    task.spawn(function()
        task.wait(0.15) -- aguarda ProximityPrompt se estabilizar
        if IsItemFreeInWorld(obj) and ItemHasGround(obj) then
            GrabItem(itemName)
        end
...
local function StartAutoGrabWatcher()
    StopAutoGrabWatcher()
    -- Escuta novos itens que entram no Workspace
    AutoGrabConn = Workspace.DescendantAdded:Connect(function(obj)
        TryAutoGrab(obj)
    end)
end

-- Varre itens já existentes no mapa (chamado ao ativar um toggle)
...
TabMiniGame:Button({
    ...
    Callback = function()
        ...
        -- Checa se há chão na posição alvo (raycast a partir de alguns studs acima)
